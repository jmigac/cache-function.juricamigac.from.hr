...
class ContentfulRequest:
    ...
    def get_projects(self):
        ...
        response = requests.request("POST", self.base_url, headers=headers, data=Payload.PROJECTS_PAYLOAD)
    ...
    def get_experiences(self):
        headers = self.get_headers()
        response = requests.request("POST", self.base_url, headers=headers, data=Payload.EXPERIENCES_PAYLOAD)
        response_data = response.json()
        response_field = response_data['data']
